...
CACHE_DURATIONS = {
    'realtime': 300,      # 实时行情缓存5分钟
    'kline': 3600,        # K线数据缓存1小时
    'financial': 86400,   # 财务数据缓存24小时
    'ranking': 120        # 榜单数据缓存2分钟
}
...
def get_realtime_data():
    ...
    try:
        # 设置超时限制
        original_timeout = requests.gettimeout() if hasattr(requests, 'gettimeout') else None
        requests.settimeout(AKSHARE_TIMEOUT)
        
        data = ak.stock_zh_a_spot()
        set_cache(cache_key, data)
        
        if original_timeout is not None:
            requests.settimeout(original_timeout)
        
        return data
    except requests.exceptions.Timeout:
        st.warning('⚠️ 行情延迟，展示缓存数据')
        return cached_data
    except Exception as e:
...
def calculate_ranking_score(df):
    """
    智能榜单打分算法
    权重分配：近2日涨跌幅(60%) + 股价趋势(25%) + 估值程度(15%)
    """
    df = df.copy()
    df['score'] = 0
    ...
    # 股价趋势权重25%（使用随机模拟，实际应基于K线趋势）
    df['score'] += np.random.uniform(0, 0.25, len(df))
    
    # 估值程度权重15%（使用随机模拟，实际应基于估值偏离）
